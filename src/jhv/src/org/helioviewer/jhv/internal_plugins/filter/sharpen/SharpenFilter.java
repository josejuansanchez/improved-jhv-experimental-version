        ...
        try {
            if (data.getImageTransport() instanceof Byte8ImageTransport) {
                ...
                for (int i = 0; i < pixelData.length; i++) {
                    pixelData[i] = (byte) Math.min(Math.max((1.0f + weighting) * (pixelData[i] & 0xFF) - weighting * convolveY[i], 0), 0xFF);
                }
                ...
                // unsharp masking
                for (int i = 0; i < pixelData.length; i++) {
                    pixelData[i] = (short) Math.min(Math.max((1.0f + weighting) * (pixelData[i] & mask) - weighting * convolveY[i], 0), 0xFFFF);
                }
                ...
                DataBufferInt tmp = dataBufferPool.reserveDataBufferInt();
                int[] channel = tmp.getData();

                // perform for each color channel
                for (int c = 0; c < 3; c++) {
                    for (int i = 0; i < pixelData.length; i++) {
                        channel[i] = (pixelData[i] >>> c * 8) & 0xFF;
                        pixelData[i] &= 0xFFFFFFFF ^ (0xFF << c * 8);
                    }
                    ...
                    // unsharp masking
                    for (int i = 0; i < pixelData.length; i++) {
                        pixelData[i] |= (((int) Math.min(Math.max((1.0f + weighting) * (channel[i] & 0xFF) - weighting * convolveY[i], 0), 0xFF)) << (c * 8));
                    }
                }

                dataBufferPool.releaseDataBuffer(tmp);
