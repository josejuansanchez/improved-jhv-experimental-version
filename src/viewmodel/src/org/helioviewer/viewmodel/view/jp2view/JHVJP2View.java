...
import org.helioviewer.viewmodel.view.jp2view.J2KReader.ReaderReasons;
...
public class JHVJP2View extends AbstractView implements JP2View, ViewportView, RegionView, MetaDataView, SubimageDataView, ImageInfoView {
    ...
    final ReasonSignal<ReaderReasons> readerSignal = new ReasonSignal<ReaderReasons>();
    ...
    protected DataBufferPool dataBufferPool;
    
    public static final int INITIAL_VIEWPORT_WIDHT_MAINVIEW = 1024;
    public static final int INITIAL_VIEWPORT_HEIGHT_MAINVIEW = 1024;
    
    public static final int INITIAL_VIEWPORT_WIDHT_MINIVIEW = 256;
    public static final int INITIAL_VIEWPORT_HEIGHT_MINIVIEW = 256;
    ...
    /**
     * Returns the J2KRender object managed by this class
     * 
     * @return
     */
    public J2KRender getRender() {
    	return render;
    }
    ...
    public void setJP2Image(JP2Image newJP2Image) {
        ...
        if (viewport == null) {
        	if (isMainView)
        		viewport = StaticViewport.createAdaptedViewport(INITIAL_VIEWPORT_WIDHT_MAINVIEW, INITIAL_VIEWPORT_HEIGHT_MAINVIEW);
        	else
        		viewport = StaticViewport.createAdaptedViewport(INITIAL_VIEWPORT_WIDHT_MINIVIEW, INITIAL_VIEWPORT_HEIGHT_MINIVIEW);
        }
     ...
     * {@inheritDoc}
     */
    public synchronized ImageData getSubimageData(boolean readOnly) {
        if (readOnly || imageData == null) {
            return imageData;
        } else {
            DataBufferPool dataBufferPool = ((JavaBufferedImageData) imageData).getDataBufferPool();
            if (imageData instanceof SingleChannelByte8ImageData) {
                BufferedImage bi = dataBufferPool.reserveBufferedImageByte();
                ((JavaBufferedImageData) imageData).getBufferedImage().copyData(bi.getRaster());
                return new SingleChannelByte8ImageData(imageData, bi);
            } else if (imageData instanceof SingleChannelShortImageData) {
                BufferedImage bi = dataBufferPool.reserveBufferedImageShort(((Short16ImageTransport) imageData.getImageFormat()).getNumBitsPerPixel());
                ((JavaBufferedImageData) imageData).getBufferedImage().copyData(bi.getRaster());
                return new SingleChannelShortImageData(imageData, bi);
            } else if (imageData instanceof ARGBInt32ImageData) {
                BufferedImage bi = dataBufferPool.reserveBufferedImageInt();
                ((JavaBufferedImageData) imageData).getBufferedImage().copyData(bi.getRaster());

                return new ARGBInt32ImageData(imageData, bi);
            } else {
                return imageData;
            }
        }
    ...
    protected void startDecoding() {    		
    	reader.start();
    	render.start();
        readerSignal.signal(ReaderReasons.OTHER);
    }
    ...
    void setSubimageData(ImageData newImageData, SubImage roi, int compositionLayer) {
    	synchronized (this) {
            if (imageData != null && ((JavaBufferedImageData) newImageData).getDataBufferPool() != null) {
                ((JavaBufferedImageData) newImageData).getDataBufferPool().releaseBufferedImage(((JavaBufferedImageData) newImageData).getBufferedImage());
            }
            if (newImageData != null && (dataBufferPool == null || dataBufferPool.getWidth() != newImageData.getWidth() || dataBufferPool.getHeight() != newImageData.getHeight())) {
                dataBufferPool = new DataBufferPool(newImageData.getWidth(), newImageData.getHeight());
            }
            if (newImageData != null) {
                ((JavaBufferedImageData) newImageData).setDataBufferPool(dataBufferPool);
            }
