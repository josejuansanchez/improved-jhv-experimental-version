...
public class BufferedImageComponentView extends AbstractComponentView {
    ...
    private DataBufferPool bufferPool;
    ...
    private synchronized void updatePanel() {
        ...
        JavaBufferedImageData latestImage = ViewHelper.getImageDataAdapter(view, JavaBufferedImageData.class);

        // pass latest image data to the panel where data shall be displayed
        if (latestImage != null) {
            bufferPool = latestImage.getDataBufferPool();
            BufferedImage latestImageCopy = bufferPool.reserveBufferedImageInt();
            latestImage.getBufferedImage().copyData(latestImageCopy.getRaster());
            bufferPool.releaseBufferedImage(latestImage.getBufferedImage());
            JavaBufferedImageData oldData = imageData;
            imageData = new ARGBInt32ImageData(latestImage, latestImageCopy);
            javaImagePanel.setImage(imageData.getBufferedImage());
            if (oldData != null)
                oldData.getDataBufferPool().releaseBufferedImage(oldData.getBufferedImage());
