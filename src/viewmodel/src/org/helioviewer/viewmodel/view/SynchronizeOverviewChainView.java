...
public class SynchronizeOverviewChainView extends AbstractSynchronizeChainView {
    ...
    private void layerChanged(LayerChangedReason aLayerChangedReason) {
    	...
    	if (aLayerChangedReason.getLayerChangeType() == LayerChangeType.LAYER_ADDED) {
            ...
            ImageInfoView overviewImageView = addLayer(aLayerChangedReason.getSubView());

            if (overviewImageView != null) {
                // memorize relation of views between different view chains
                viewRelations.put(ViewHelper.getViewAdapter(aLayerChangedReason.getSubView(), ImageInfoView.class), overviewImageView);
            }
     ...
     *         be created.
     */
    private ImageInfoView addLayer(View view) {
        ...
        ImageInfoView newImageView = viewFactory.createViewFromSource(sourceImageView, true);

        // if no major filter view found
        if (first == null) {
            getAdapter(LayeredView.class).addLayer(newImageView);
        } else {
            last.setView(newImageView);
     ...
     * 
     * @param aView
     *            View of the sub chain. The sub chain will be removed from the
     *            layered view.
     * */
    private void removeLayer(View aView) {
